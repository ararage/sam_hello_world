...
```bash
aws s3 mb s3://sam-project --region us-east-1
```

## Important:  
Edit the samconfig.toml and delete and invalidate the resolve_s3 property

### Package project
```bash
sam package --s3-bucket sam-project --output-template-file output.yaml
```
...
### Inspect Stack Status as a Table
```bash
aws cloudformation describe-stacks --stack-name sam-test-1 --output table
```
...
### Searching Error Logs From 1 month ago 
```bash
...
### Delete Stack 
```bash
aws cloudformation delete-stack --stack-name sam-test-1 
```

### Delete Bucket Content 
```bash
aws s3 rm s3://sam-project-pepe --recursive --profile educative 
```
...
```
Note : For Every new change you have to run sam build 

## Debug with VS Code

Inside the .vscode directory create the launch.json with the next content.
